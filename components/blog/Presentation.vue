...
const posts = ref(data.value?.filter((post) => post.path.startsWith(`/${locale.value}`)) ?? []);
        ...
        <div class="mx-auto mt-16 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-20 lg:mx-0 lg:max-w-none lg:grid-cols-3">
            <ClientOnly>
                <article v-for="post in posts" :key="post.title"
                    ...
                    <NuxtLink class="flex flex-col items-start h-full justify-between" :href="`${post.path}`">
