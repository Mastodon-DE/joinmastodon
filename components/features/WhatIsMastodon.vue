<template>
	<div
		class="relative isolate overflow-hidden px-6 py-24 sm:py-32 lg:overflow-visible lg:px-0">
		<div
			class="mx-auto grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 lg:mx-0 lg:max-w-none lg:grid-cols-2 lg:items-start lg:gap-y-10">
			<div
				class="lg:col-span-2 lg:col-start-1 lg:row-start-1 lg:mx-auto lg:grid lg:w-full lg:max-w-7xl lg:grid-cols-2 lg:gap-x-8 lg:px-8">
				<div class="lg:pr-4">
					<div class="lg:max-w-lg">
						<h1
							class="mt-2 text-3xl font-bold tracking-tight text-gray-50 sm:text-4xl">
							{{ $t("WhatisMastodonHeading") }}
						</h1>
						<p class="mt-6 text-xl leading-8 text-gray-200">
							{{ $t("WhatisMastodonExplaination") }}
						</p>
					</div>
				</div>
			</div>
			<div
				class="-ml-12 -mt-12 p-12 lg:sticky lg:top-20 lg:col-start-2 lg:row-span-2 lg:row-start-1">
				<iframe
					class="w-[48rem] max-w-full aspect-[560/315] rounded-xl bg-gray-900 shadow-xl ring-1 ring-gray-400/10 sm:w-[57rem]"
					src="https://www.youtube-nocookie.com/embed/IPSbNdBmWKE?si=c-S-vptRYTTYKl4t"
					title="YouTube video player"
					frameborder="0"
					allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
					allowfullscreen></iframe>
			</div>
			<div
				class="lg:col-span-2 lg:col-start-1 lg:row-start-2 lg:mx-auto lg:grid lg:w-full lg:max-w-7xl lg:grid-cols-2 lg:gap-x-8 lg:px-8">
				<div class="lg:pr-4">
					<div
						class="max-w-xl text-base leading-7 text-gray-200 lg:max-w-lg">
						<p>
							{{ $t("WhatisMastodonExplaination2") }}
						</p>
						<p class="mt-8">
							{{ $t("WhatisMastodonExplaination3") }}
						</p>
						<h2
							class="mt-16 text-2xl font-bold tracking-tight text-gray-50">
							{{ $t("WhatisMastodonHeading2") }}
						</h2>
						<p class="mt-6">
							{{ $t("WhatisMastodonExplaination4") }}
						</p>
					</div>
				</div>
			</div>
		</div>
	</div>
</template>
