...
<template>
    <header :class="['bg-dark-500/40 backdrop-blur-lg']" class="fixed top-0 inset-x-0 z-50">
        <nav class="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8" aria-label="Global">
            ...
            <div class="flex lg:hidden">
                <button type="button"
                    ...
                    <iconify-icon width="unset" icon="tabler:menu-2" class="size-6" aria-hidden="true" />
                </button>
            </div>
            <HeadlessPopoverGroup class="hidden lg:flex lg:gap-x-12">
                <HeadlessPopover class="relative">
                    <HeadlessPopoverButton
                        ...
                        {{ $t("navbar.aboutUs") }}
                        <iconify-icon width="unset" icon="tabler:chevron-down" class="h-5 w-5 flex-none text-gray-500"
                            aria-hidden="true" />
                    </HeadlessPopoverButton>
                    ...
                    <transition enter-active-class="transition ease-out duration-200"
                        ...
                        <HeadlessPopoverPanel
                            ...
                            <a v-for="item in aboutUs" :key="item.name" :href="item.href"
                ...
                <a href="https://status.mastodon.de" class="text-sm font-semibold leading-6 text-gray-50">{{
                    $t("navbar.status") }}</a>

                <a href="/en/donate" class="text-sm font-semibold leading-6 text-gray-50">{{ $t("navbar.donate") }}</a>
                <NuxtLink href="/en/contact" class="text-sm font-semibold leading-6 text-gray-50">{{
                    $t("navbar.contact")
                    }}</NuxtLink>
            </HeadlessPopoverGroup>
            <div class="hidden lg:flex lg:flex-1 lg:justify-end gap-4 lg:items-center">
                <HeadlessMenu as="div" class="relative inline-block text-left">
                    <div>
                        <HeadlessMenuButton class="rounded-md group font-semibold leading-6 text-white">
                            <iconify-icon width="unset" icon="tabler:language" class="size-6" />
                        </HeadlessMenuButton>
                    </div>
                ...
                <a href="https://mastodon.de" class="text-sm group font-semibold leading-6 text-gray-50">{{
                    $t("navbar.explore") }}
                    <ButtonsAnimatedArrow />
                </a>
            </div>
        ...
        <HeadlessTransitionRoot appear :show="mobileMenuOpen" as="template">
            <HeadlessDialog as="div" class="lg:hidden" @close="mobileMenuOpen = false">
                <div class="fixed inset-0 z-10" />

                <HeadlessTransitionChild as="template" enter-active-class="transition ease-out duration-200"
                    ...
                    <HeadlessDialogPanel
                        class="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-dark-700 px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-100/10">
                        <div class="flex items-center justify-between">
                            ...
                            <button type="button" class="-m-2.5 rounded-md p-2.5 text-gray-200"
                                ...
                                <iconify-icon width="unset" icon="tabler:x" class="size-6" aria-hidden="true" />
                            </button>
                        </div>
                                ...
                                <div class="space-y-2 py-6">
                                    <HeadlessDisclosure v-slot="{ open }" as="div" class="-mx-3">
                                        <HeadlessDisclosureButton
                                            ...
                                            {{ $t("aboutUs") }}
                                            <iconify-icon width="unset" icon="tabler:chevron-down" :class="[
                                                open ? 'rotate-180' : '',
                                                'h-5 w-5 flex-none',
                                            ]" aria-hidden="true" />
                                        </HeadlessDisclosureButton>
                                        <HeadlessDisclosurePanel class="mt-2 space-y-2">
                                            <HeadlessDisclosureButton v-for="item in aboutUs" :key="item.name" as="a"
                                                :href="item.href"
